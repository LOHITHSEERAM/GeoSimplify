package org.example.pojo;
class Properties {


    private String addrCity;
    private String addrCountry;
    private String addrHousenumber;
    private String addrPostcode;
    private String addrStreet;
    private String amenity;
    private String building;
    private String phone;
    private String tourism;
    private String name;
    private String indoorSeating;
    private String outdoorSeating;
    private String nameKn;
    private String smoking;
    private String checkDate;
    private String cuisine;
    private String level;
    private String operator;
    private String toilets;
    private String toiletsWheelchair;
    private String wheelchair;
    private String rastorant;
    private String openingHours;
    private String delivery;
    private String takeaway;
    private String buildingLevels;
    private String roofLevels;
    private String internetAccess;
    private String shop;
    private String roofColour;
    private String website;
    private String addrFloor;
    private String bar;
    private String branch;
    private String brand;
    private String capacity;
    private String dietVegetarian;
    private String dietPescetarian;
    private String nameEn;
    private String contactMobile;
    private String contactPhone;
    private String airConditioning;
    private String microbrewery;
    private String brandWikidata;
    private String sourceAddrHousenumber;
    private String description;
    private String address;
    private String driveThrough;
    private String reservation;
    private String toiletsPosition;
    private String note;
    private String operationalStatus;
    private String paymentCreditCards;
    private String paymentDebitCards;
    private String buildingLevelsUnderground;
    private String roofShape;
    private String internetAccessFee;
    private String internetAccessSsid;
    private String addrHamlet;
    private String addrSuburb;
    private String email;
    private String source;
    private String minAge;
    private String access;
    private String operatorType;
    private String brandKs;
    private String brandPa;
    private String brandUr;
    private String brandWikipedia;
    private String nameKs;
    private String namePa;
    private String nameUr;
    private String craft;
    private String addrDistrict;
    private String contactInstagram;
    private String altName;
    private String createdBy;
    private String dietNonVegetarian;
    private String food;
    private String officialName;
    private String startDate;
    private String wikidata;
    private String paymentMastercard;
    private String paymentVisa;
    private String addrHousename;
    private String shortName;
    private String wikimediaCommons;
    private String wikipedia;
    private String speciality;
    private String addrFull;
    private String addrState;
    private String fax;
    private String dietHalal;
    private String highchair;
    private String paymentCash;
    private String paymentUpi;
    private String dietVegan;
    private String happycowId;
    private String oldName;
    private String officialNamePa;
    private String officialNameUr;
    private String wifi;
    private String brandWikipediaPa;
    private String brandWikipediaUr;
    private String designation;
    private String phone1;
    private String phone2;
    private String intName;
    private String checkDateDietHalal;
    private String surveyDate;
    private String userDefined;
    private String contactWhatsapp;
    private String cuisine1;
    private String contactFacebook;
    private String nameHi;
    private String working;
    private String service;
    private String coffee;
    private String drinkCoffee;
    private String drinkJuice;
    private String addrStreet1;
    private String brewery;
    private String contactEmail;
    private String contactWebsite;
    private String driveIn;
    private String organic;
    private String amenity1;
    private String refVatin;
    private String nameMl;
    private String mapillary;
    private String nameZh;
    private String dietLactoVegetarian;
    private String addrPlace;
    private String ele;
    private String paymentElectronicPurses;
    private String floor;
    private String mobile;
    private String openingHoursCovid19;
    private String dineIn;
    private String dietPlantBased;
    private String dietMeat;
    private String paymentBitcoincash;
    private String dietSeafood;
    private String image;
    private String brandDe;
    private String brandFr;
    private String nameDe;
    private String nameFr;
    private String checkDateSmoking;
    private String fixme;
    private String contactPinterest;
    private String addrNumber;
    private String checkDateOpeningHours;
    private String openingHoursSigned;
    private String addrUnit;
    private String contactTwitter;
    private String toiletsAccess;
    private String contactYoutube;
    private String buildingFloor;
    private String realFire;
    private String cafe;
    private String paymentCards;
    private String nameKo;



    private String id;

    private String buildingPart;
    private String government;
    private String height;
    private String layer;


    private String nameTe;

    public String getAddrCity() {
        return addrCity;
    }

    public void setAddrCity(String addrCity) {
        this.addrCity = addrCity;
    }

    public String getAddrCountry() {
        return addrCountry;
    }

    public void setAddrCountry(String addrCountry) {
        this.addrCountry = addrCountry;
    }

    public String getAddrHousenumber() {
        return addrHousenumber;
    }

    public void setAddrHousenumber(String addrHousenumber) {
        this.addrHousenumber = addrHousenumber;
    }

    public String getAddrPostcode() {
        return addrPostcode;
    }

    public void setAddrPostcode(String addrPostcode) {
        this.addrPostcode = addrPostcode;
    }

    public String getAddrStreet() {
        return addrStreet;
    }

    public void setAddrStreet(String addrStreet) {
        this.addrStreet = addrStreet;
    }

    public String getAmenity() {
        return amenity;
    }

    public void setAmenity(String amenity) {
        this.amenity = amenity;
    }

    public String getBuilding() {
        return building;
    }

    public void setBuilding(String building) {
        this.building = building;
    }

    public String getPhone() {
        return phone;
    }

    public void setPhone(String phone) {
        this.phone = phone;
    }

    public String getTourism() {
        return tourism;
    }

    public void setTourism(String tourism) {
        this.tourism = tourism;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getIndoorSeating() {
        return indoorSeating;
    }

    public void setIndoorSeating(String indoorSeating) {
        this.indoorSeating = indoorSeating;
    }

    public String getOutdoorSeating() {
        return outdoorSeating;
    }

    public void setOutdoorSeating(String outdoorSeating) {
        this.outdoorSeating = outdoorSeating;
    }

    public String getNameKn() {
        return nameKn;
    }

    public void setNameKn(String nameKn) {
        this.nameKn = nameKn;
    }

    public String getSmoking() {
        return smoking;
    }

    public void setSmoking(String smoking) {
        this.smoking = smoking;
    }

    public String getCheckDate() {
        return checkDate;
    }

    public void setCheckDate(String checkDate) {
        this.checkDate = checkDate;
    }

    public String getCuisine() {
        return cuisine;
    }

    public void setCuisine(String cuisine) {
        this.cuisine = cuisine;
    }

    public String getLevel() {
        return level;
    }

    public void setLevel(String level) {
        this.level = level;
    }

    public String getOperator() {
        return operator;
    }

    public void setOperator(String operator) {
        this.operator = operator;
    }

    public String getToilets() {
        return toilets;
    }

    public void setToilets(String toilets) {
        this.toilets = toilets;
    }

    public String getToiletsWheelchair() {
        return toiletsWheelchair;
    }

    public void setToiletsWheelchair(String toiletsWheelchair) {
        this.toiletsWheelchair = toiletsWheelchair;
    }

    public String getWheelchair() {
        return wheelchair;
    }

    public void setWheelchair(String wheelchair) {
        this.wheelchair = wheelchair;
    }

    public String getRastorant() {
        return rastorant;
    }

    public void setRastorant(String rastorant) {
        this.rastorant = rastorant;
    }

    public String getOpeningHours() {
        return openingHours;
    }

    public void setOpeningHours(String openingHours) {
        this.openingHours = openingHours;
    }

    public String getDelivery() {
        return delivery;
    }

    public void setDelivery(String delivery) {
        this.delivery = delivery;
    }

    public String getTakeaway() {
        return takeaway;
    }

    public void setTakeaway(String takeaway) {
        this.takeaway = takeaway;
    }

    public String getBuildingLevels() {
        return buildingLevels;
    }

    public void setBuildingLevels(String buildingLevels) {
        this.buildingLevels = buildingLevels;
    }

    public String getRoofLevels() {
        return roofLevels;
    }

    public void setRoofLevels(String roofLevels) {
        this.roofLevels = roofLevels;
    }

    public String getInternetAccess() {
        return internetAccess;
    }

    public void setInternetAccess(String internetAccess) {
        this.internetAccess = internetAccess;
    }

    public String getShop() {
        return shop;
    }

    public void setShop(String shop) {
        this.shop = shop;
    }

    public String getRoofColour() {
        return roofColour;
    }

    public void setRoofColour(String roofColour) {
        this.roofColour = roofColour;
    }

    public String getWebsite() {
        return website;
    }

    public void setWebsite(String website) {
        this.website = website;
    }

    public String getAddrFloor() {
        return addrFloor;
    }

    public void setAddrFloor(String addrFloor) {
        this.addrFloor = addrFloor;
    }

    public String getBar() {
        return bar;
    }

    public void setBar(String bar) {
        this.bar = bar;
    }

    public String getBranch() {
        return branch;
    }

    public void setBranch(String branch) {
        this.branch = branch;
    }

    public String getBrand() {
        return brand;
    }

    public void setBrand(String brand) {
        this.brand = brand;
    }

    public String getCapacity() {
        return capacity;
    }

    public void setCapacity(String capacity) {
        this.capacity = capacity;
    }

    public String getDietVegetarian() {
        return dietVegetarian;
    }

    public void setDietVegetarian(String dietVegetarian) {
        this.dietVegetarian = dietVegetarian;
    }

    public String getDietPescetarian() {
        return dietPescetarian;
    }

    public void setDietPescetarian(String dietPescetarian) {
        this.dietPescetarian = dietPescetarian;
    }

    public String getNameEn() {
        return nameEn;
    }

    public void setNameEn(String nameEn) {
        this.nameEn = nameEn;
    }

    public String getContactMobile() {
        return contactMobile;
    }

    public void setContactMobile(String contactMobile) {
        this.contactMobile = contactMobile;
    }

    public String getContactPhone() {
        return contactPhone;
    }

    public void setContactPhone(String contactPhone) {
        this.contactPhone = contactPhone;
    }

    public String getAirConditioning() {
        return airConditioning;
    }

    public void setAirConditioning(String airConditioning) {
        this.airConditioning = airConditioning;
    }

    public String getMicrobrewery() {
        return microbrewery;
    }

    public void setMicrobrewery(String microbrewery) {
        this.microbrewery = microbrewery;
    }

    public String getBrandWikidata() {
        return brandWikidata;
    }

    public void setBrandWikidata(String brandWikidata) {
        this.brandWikidata = brandWikidata;
    }

    public String getSourceAddrHousenumber() {
        return sourceAddrHousenumber;
    }

    public void setSourceAddrHousenumber(String sourceAddrHousenumber) {
        this.sourceAddrHousenumber = sourceAddrHousenumber;
    }

    public String getDescription() {
        return description;
    }

    public void setDescription(String description) {
        this.description = description;
    }

    public String getAddress() {
        return address;
    }

    public void setAddress(String address) {
        this.address = address;
    }

    public String getDriveThrough() {
        return driveThrough;
    }

    public void setDriveThrough(String driveThrough) {
        this.driveThrough = driveThrough;
    }

    public String getReservation() {
        return reservation;
    }

    public void setReservation(String reservation) {
        this.reservation = reservation;
    }

    public String getToiletsPosition() {
        return toiletsPosition;
    }

    public void setToiletsPosition(String toiletsPosition) {
        this.toiletsPosition = toiletsPosition;
    }

    public String getNote() {
        return note;
    }

    public void setNote(String note) {
        this.note = note;
    }

    public String getOperationalStatus() {
        return operationalStatus;
    }

    public void setOperationalStatus(String operationalStatus) {
        this.operationalStatus = operationalStatus;
    }

    public String getPaymentCreditCards() {
        return paymentCreditCards;
    }

    public void setPaymentCreditCards(String paymentCreditCards) {
        this.paymentCreditCards = paymentCreditCards;
    }

    public String getPaymentDebitCards() {
        return paymentDebitCards;
    }

    public void setPaymentDebitCards(String paymentDebitCards) {
        this.paymentDebitCards = paymentDebitCards;
    }

    public String getBuildingLevelsUnderground() {
        return buildingLevelsUnderground;
    }

    public void setBuildingLevelsUnderground(String buildingLevelsUnderground) {
        this.buildingLevelsUnderground = buildingLevelsUnderground;
    }

    public String getRoofShape() {
        return roofShape;
    }

    public void setRoofShape(String roofShape) {
        this.roofShape = roofShape;
    }

    public String getInternetAccessFee() {
        return internetAccessFee;
    }

    public void setInternetAccessFee(String internetAccessFee) {
        this.internetAccessFee = internetAccessFee;
    }

    public String getInternetAccessSsid() {
        return internetAccessSsid;
    }

    public void setInternetAccessSsid(String internetAccessSsid) {
        this.internetAccessSsid = internetAccessSsid;
    }

    public String getAddrHamlet() {
        return addrHamlet;
    }

    public void setAddrHamlet(String addrHamlet) {
        this.addrHamlet = addrHamlet;
    }

    public String getAddrSuburb() {
        return addrSuburb;
    }

    public void setAddrSuburb(String addrSuburb) {
        this.addrSuburb = addrSuburb;
    }

    public String getEmail() {
        return email;
    }

    public void setEmail(String email) {
        this.email = email;
    }

    public String getSource() {
        return source;
    }

    public void setSource(String source) {
        this.source = source;
    }

    public String getMinAge() {
        return minAge;
    }

    public void setMinAge(String minAge) {
        this.minAge = minAge;
    }

    public String getAccess() {
        return access;
    }

    public void setAccess(String access) {
        this.access = access;
    }

    public String getOperatorType() {
        return operatorType;
    }

    public void setOperatorType(String operatorType) {
        this.operatorType = operatorType;
    }

    public String getBrandKs() {
        return brandKs;
    }

    public void setBrandKs(String brandKs) {
        this.brandKs = brandKs;
    }

    public String getBrandPa() {
        return brandPa;
    }

    public void setBrandPa(String brandPa) {
        this.brandPa = brandPa;
    }

    public String getBrandUr() {
        return brandUr;
    }

    public void setBrandUr(String brandUr) {
        this.brandUr = brandUr;
    }

    public String getBrandWikipedia() {
        return brandWikipedia;
    }

    public void setBrandWikipedia(String brandWikipedia) {
        this.brandWikipedia = brandWikipedia;
    }

    public String getNameKs() {
        return nameKs;
    }

    public void setNameKs(String nameKs) {
        this.nameKs = nameKs;
    }

    public String getNamePa() {
        return namePa;
    }

    public void setNamePa(String namePa) {
        this.namePa = namePa;
    }

    public String getNameUr() {
        return nameUr;
    }

    public void setNameUr(String nameUr) {
        this.nameUr = nameUr;
    }

    public String getCraft() {
        return craft;
    }

    public void setCraft(String craft) {
        this.craft = craft;
    }

    public String getAddrDistrict() {
        return addrDistrict;
    }

    public void setAddrDistrict(String addrDistrict) {
        this.addrDistrict = addrDistrict;
    }

    public String getContactInstagram() {
        return contactInstagram;
    }

    public void setContactInstagram(String contactInstagram) {
        this.contactInstagram = contactInstagram;
    }

    public String getAltName() {
        return altName;
    }

    public void setAltName(String altName) {
        this.altName = altName;
    }

    public String getCreatedBy() {
        return createdBy;
    }

    public void setCreatedBy(String createdBy) {
        this.createdBy = createdBy;
    }

    public String getDietNonVegetarian() {
        return dietNonVegetarian;
    }

    public void setDietNonVegetarian(String dietNonVegetarian) {
        this.dietNonVegetarian = dietNonVegetarian;
    }

    public String getFood() {
        return food;
    }

    public void setFood(String food) {
        this.food = food;
    }

    public String getOfficialName() {
        return officialName;
    }

    public void setOfficialName(String officialName) {
        this.officialName = officialName;
    }

    public String getStartDate() {
        return startDate;
    }

    public void setStartDate(String startDate) {
        this.startDate = startDate;
    }

    public String getWikidata() {
        return wikidata;
    }

    public void setWikidata(String wikidata) {
        this.wikidata = wikidata;
    }

    public String getPaymentMastercard() {
        return paymentMastercard;
    }

    public void setPaymentMastercard(String paymentMastercard) {
        this.paymentMastercard = paymentMastercard;
    }

    public String getPaymentVisa() {
        return paymentVisa;
    }

    public void setPaymentVisa(String paymentVisa) {
        this.paymentVisa = paymentVisa;
    }

    public String getAddrHousename() {
        return addrHousename;
    }

    public void setAddrHousename(String addrHousename) {
        this.addrHousename = addrHousename;
    }

    public String getShortName() {
        return shortName;
    }

    public void setShortName(String shortName) {
        this.shortName = shortName;
    }

    public String getWikimediaCommons() {
        return wikimediaCommons;
    }

    public void setWikimediaCommons(String wikimediaCommons) {
        this.wikimediaCommons = wikimediaCommons;
    }

    public String getWikipedia() {
        return wikipedia;
    }

    public void setWikipedia(String wikipedia) {
        this.wikipedia = wikipedia;
    }

    public String getSpeciality() {
        return speciality;
    }

    public void setSpeciality(String speciality) {
        this.speciality = speciality;
    }

    public String getAddrFull() {
        return addrFull;
    }

    public void setAddrFull(String addrFull) {
        this.addrFull = addrFull;
    }

    public String getAddrState() {
        return addrState;
    }

    public void setAddrState(String addrState) {
        this.addrState = addrState;
    }

    public String getFax() {
        return fax;
    }

    public void setFax(String fax) {
        this.fax = fax;
    }

    public String getDietHalal() {
        return dietHalal;
    }

    public void setDietHalal(String dietHalal) {
        this.dietHalal = dietHalal;
    }

    public String getHighchair() {
        return highchair;
    }

    public void setHighchair(String highchair) {
        this.highchair = highchair;
    }

    public String getPaymentCash() {
        return paymentCash;
    }

    public void setPaymentCash(String paymentCash) {
        this.paymentCash = paymentCash;
    }

    public String getPaymentUpi() {
        return paymentUpi;
    }

    public void setPaymentUpi(String paymentUpi) {
        this.paymentUpi = paymentUpi;
    }

    public String getDietVegan() {
        return dietVegan;
    }

    public void setDietVegan(String dietVegan) {
        this.dietVegan = dietVegan;
    }

    public String getHappycowId() {
        return happycowId;
    }

    public void setHappycowId(String happycowId) {
        this.happycowId = happycowId;
    }

    public String getOldName() {
        return oldName;
    }

    public void setOldName(String oldName) {
        this.oldName = oldName;
    }

    public String getOfficialNamePa() {
        return officialNamePa;
    }

    public void setOfficialNamePa(String officialNamePa) {
        this.officialNamePa = officialNamePa;
    }

    public String getOfficialNameUr() {
        return officialNameUr;
    }

    public void setOfficialNameUr(String officialNameUr) {
        this.officialNameUr = officialNameUr;
    }

    public String getWifi() {
        return wifi;
    }

    public void setWifi(String wifi) {
        this.wifi = wifi;
    }

    public String getBrandWikipediaPa() {
        return brandWikipediaPa;
    }

    public void setBrandWikipediaPa(String brandWikipediaPa) {
        this.brandWikipediaPa = brandWikipediaPa;
    }

    public String getBrandWikipediaUr() {
        return brandWikipediaUr;
    }

    public void setBrandWikipediaUr(String brandWikipediaUr) {
        this.brandWikipediaUr = brandWikipediaUr;
    }

    public String getDesignation() {
        return designation;
    }

    public void setDesignation(String designation) {
        this.designation = designation;
    }

    public String getPhone1() {
        return phone1;
    }

    public void setPhone1(String phone1) {
        this.phone1 = phone1;
    }

    public String getPhone2() {
        return phone2;
    }

    public void setPhone2(String phone2) {
        this.phone2 = phone2;
    }

    public String getIntName() {
        return intName;
    }

    public void setIntName(String intName) {
        this.intName = intName;
    }

    public String getCheckDateDietHalal() {
        return checkDateDietHalal;
    }

    public void setCheckDateDietHalal(String checkDateDietHalal) {
        this.checkDateDietHalal = checkDateDietHalal;
    }

    public String getSurveyDate() {
        return surveyDate;
    }

    public void setSurveyDate(String surveyDate) {
        this.surveyDate = surveyDate;
    }

    public String getUserDefined() {
        return userDefined;
    }

    public void setUserDefined(String userDefined) {
        this.userDefined = userDefined;
    }

    public String getContactWhatsapp() {
        return contactWhatsapp;
    }

    public void setContactWhatsapp(String contactWhatsapp) {
        this.contactWhatsapp = contactWhatsapp;
    }

    public String getCuisine1() {
        return cuisine1;
    }

    public void setCuisine1(String cuisine1) {
        this.cuisine1 = cuisine1;
    }

    public String getContactFacebook() {
        return contactFacebook;
    }

    public void setContactFacebook(String contactFacebook) {
        this.contactFacebook = contactFacebook;
    }

    public String getNameHi() {
        return nameHi;
    }

    public void setNameHi(String nameHi) {
        this.nameHi = nameHi;
    }

    public String getWorking() {
        return working;
    }

    public void setWorking(String working) {
        this.working = working;
    }

    public String getService() {
        return service;
    }

    public void setService(String service) {
        this.service = service;
    }

    public String getCoffee() {
        return coffee;
    }

    public void setCoffee(String coffee) {
        this.coffee = coffee;
    }

    public String getDrinkCoffee() {
        return drinkCoffee;
    }

    public void setDrinkCoffee(String drinkCoffee) {
        this.drinkCoffee = drinkCoffee;
    }

    public String getDrinkJuice() {
        return drinkJuice;
    }

    public void setDrinkJuice(String drinkJuice) {
        this.drinkJuice = drinkJuice;
    }

    public String getAddrStreet1() {
        return addrStreet1;
    }

    public void setAddrStreet1(String addrStreet1) {
        this.addrStreet1 = addrStreet1;
    }

    public String getBrewery() {
        return brewery;
    }

    public void setBrewery(String brewery) {
        this.brewery = brewery;
    }

    public String getContactEmail() {
        return contactEmail;
    }

    public void setContactEmail(String contactEmail) {
        this.contactEmail = contactEmail;
    }

    public String getContactWebsite() {
        return contactWebsite;
    }

    public void setContactWebsite(String contactWebsite) {
        this.contactWebsite = contactWebsite;
    }

    public String getDriveIn() {
        return driveIn;
    }

    public void setDriveIn(String driveIn) {
        this.driveIn = driveIn;
    }

    public String getOrganic() {
        return organic;
    }

    public void setOrganic(String organic) {
        this.organic = organic;
    }

    public String getAmenity1() {
        return amenity1;
    }

    public void setAmenity1(String amenity1) {
        this.amenity1 = amenity1;
    }

    public String getRefVatin() {
        return refVatin;
    }

    public void setRefVatin(String refVatin) {
        this.refVatin = refVatin;
    }

    public String getNameMl() {
        return nameMl;
    }

    public void setNameMl(String nameMl) {
        this.nameMl = nameMl;
    }

    public String getMapillary() {
        return mapillary;
    }

    public void setMapillary(String mapillary) {
        this.mapillary = mapillary;
    }

    public String getNameZh() {
        return nameZh;
    }

    public void setNameZh(String nameZh) {
        this.nameZh = nameZh;
    }

    public String getDietLactoVegetarian() {
        return dietLactoVegetarian;
    }

    public void setDietLactoVegetarian(String dietLactoVegetarian) {
        this.dietLactoVegetarian = dietLactoVegetarian;
    }

    public String getAddrPlace() {
        return addrPlace;
    }

    public void setAddrPlace(String addrPlace) {
        this.addrPlace = addrPlace;
    }

    public String getEle() {
        return ele;
    }

    public void setEle(String ele) {
        this.ele = ele;
    }

    public String getPaymentElectronicPurses() {
        return paymentElectronicPurses;
    }

    public void setPaymentElectronicPurses(String paymentElectronicPurses) {
        this.paymentElectronicPurses = paymentElectronicPurses;
    }

    public String getFloor() {
        return floor;
    }

    public void setFloor(String floor) {
        this.floor = floor;
    }

    public String getMobile() {
        return mobile;
    }

    public void setMobile(String mobile) {
        this.mobile = mobile;
    }

    public String getOpeningHoursCovid19() {
        return openingHoursCovid19;
    }

    public void setOpeningHoursCovid19(String openingHoursCovid19) {
        this.openingHoursCovid19 = openingHoursCovid19;
    }

    public String getDineIn() {
        return dineIn;
    }

    public void setDineIn(String dineIn) {
        this.dineIn = dineIn;
    }

    public String getDietPlantBased() {
        return dietPlantBased;
    }

    public void setDietPlantBased(String dietPlantBased) {
        this.dietPlantBased = dietPlantBased;
    }

    public String getDietMeat() {
        return dietMeat;
    }

    public void setDietMeat(String dietMeat) {
        this.dietMeat = dietMeat;
    }

    public String getPaymentBitcoincash() {
        return paymentBitcoincash;
    }

    public void setPaymentBitcoincash(String paymentBitcoincash) {
        this.paymentBitcoincash = paymentBitcoincash;
    }

    public String getDietSeafood() {
        return dietSeafood;
    }

    public void setDietSeafood(String dietSeafood) {
        this.dietSeafood = dietSeafood;
    }

    public String getImage() {
        return image;
    }

    public void setImage(String image) {
        this.image = image;
    }

    public String getBrandDe() {
        return brandDe;
    }

    public void setBrandDe(String brandDe) {
        this.brandDe = brandDe;
    }

    public String getBrandFr() {
        return brandFr;
    }

    public void setBrandFr(String brandFr) {
        this.brandFr = brandFr;
    }

    public String getNameDe() {
        return nameDe;
    }

    public void setNameDe(String nameDe) {
        this.nameDe = nameDe;
    }

    public String getNameFr() {
        return nameFr;
    }

    public void setNameFr(String nameFr) {
        this.nameFr = nameFr;
    }

    public String getCheckDateSmoking() {
        return checkDateSmoking;
    }

    public void setCheckDateSmoking(String checkDateSmoking) {
        this.checkDateSmoking = checkDateSmoking;
    }

    public String getFixme() {
        return fixme;
    }

    public void setFixme(String fixme) {
        this.fixme = fixme;
    }

    public String getContactPinterest() {
        return contactPinterest;
    }

    public void setContactPinterest(String contactPinterest) {
        this.contactPinterest = contactPinterest;
    }

    public String getAddrNumber() {
        return addrNumber;
    }

    public void setAddrNumber(String addrNumber) {
        this.addrNumber = addrNumber;
    }

    public String getCheckDateOpeningHours() {
        return checkDateOpeningHours;
    }

    public void setCheckDateOpeningHours(String checkDateOpeningHours) {
        this.checkDateOpeningHours = checkDateOpeningHours;
    }

    public String getOpeningHoursSigned() {
        return openingHoursSigned;
    }

    public void setOpeningHoursSigned(String openingHoursSigned) {
        this.openingHoursSigned = openingHoursSigned;
    }

    public String getAddrUnit() {
        return addrUnit;
    }

    public void setAddrUnit(String addrUnit) {
        this.addrUnit = addrUnit;
    }

    public String getContactTwitter() {
        return contactTwitter;
    }

    public void setContactTwitter(String contactTwitter) {
        this.contactTwitter = contactTwitter;
    }

    public String getToiletsAccess() {
        return toiletsAccess;
    }

    public void setToiletsAccess(String toiletsAccess) {
        this.toiletsAccess = toiletsAccess;
    }

    public String getContactYoutube() {
        return contactYoutube;
    }

    public void setContactYoutube(String contactYoutube) {
        this.contactYoutube = contactYoutube;
    }

    public String getBuildingFloor() {
        return buildingFloor;
    }

    public void setBuildingFloor(String buildingFloor) {
        this.buildingFloor = buildingFloor;
    }

    public String getRealFire() {
        return realFire;
    }

    public void setRealFire(String realFire) {
        this.realFire = realFire;
    }

    public String getCafe() {
        return cafe;
    }

    public void setCafe(String cafe) {
        this.cafe = cafe;
    }

    public String getPaymentCards() {
        return paymentCards;
    }

    public void setPaymentCards(String paymentCards) {
        this.paymentCards = paymentCards;
    }

    public String getNameKo() {
        return nameKo;
    }

    public void setNameKo(String nameKo) {
        this.nameKo = nameKo;
    }

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getBuildingPart() {
        return buildingPart;
    }

    public void setBuildingPart(String buildingPart) {
        this.buildingPart = buildingPart;
    }

    public String getGovernment() {
        return government;
    }

    public void setGovernment(String government) {
        this.government = government;
    }

    public String getHeight() {
        return height;
    }

    public void setHeight(String height) {
        this.height = height;
    }

    public String getLayer() {
        return layer;
    }

    public void setLayer(String layer) {
        this.layer = layer;
    }

    public String getNameTe() {
        return nameTe;
    }

    public void setNameTe(String nameTe) {
        this.nameTe = nameTe;
    }

    public String getOffice() {
        return office;
    }

    public void setOffice(String office) {
        this.office = office;
    }

    public String getRoofMaterial() {
        return roofMaterial;
    }

    public void setRoofMaterial(String roofMaterial) {
        this.roofMaterial = roofMaterial;
    }

    public String getType() {
        return type;
    }

    public void setType(String type) {
        this.type = type;
    }

    private String office;

    private String roofMaterial;

    private String type;

}